      ...
      for (let y = 0; y < images[5].height; y += settings.algorithm.pixel.size) {
        ...
        for (let i = 0; i < 10; i++) {
          if (together > i * 26 && together < 26 + i * 26) {
            _p5.image(orderedImages[i + Math.round(Math.round(10 + Math.sin(_p5.frameCount / 25) * 10) / 2)], x + 50, y + 50, settings.algorithm.pixel.size, settings.algorithm.pixel.size)
          }
        }
      }
    }
