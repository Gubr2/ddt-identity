...
function init(_p5) {
  ...
  setInterval(() => {
    ...
    gsap.to(settings.algorithm.pixel, {
      size: 150 + (0.003 - scaleTo) * 72500,
      density: 150 + (0.003 - scaleTo) * 72500,
      duration: settings.algorithm.noise.duration,
      ease: `steps(${settings.algorithm.noise.steps})`,
    })

    gsap.to(settings.empty, {
      scale: 150 + (0.003 - scaleTo) * 72500,
      duration: settings.algorithm.noise.duration,
      ease: `steps(${settings.algorithm.noise.steps})`,
    })
    ...
    }, ((settings.algorithm.noise.duration * 1000) / settings.algorithm.noise.steps) * 2)
  ...
  function setEmpty() {
    return {
      ...
      size: {
        x: Math.round(Math.round(_p5.random(0, _p5.windowWidth)) / 100),
        y: Math.round(Math.round(_p5.random(0, _p5.windowHeight)) / 10),
      },
    }
  }
