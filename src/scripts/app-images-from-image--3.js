...
function init(_p5) {
  ...
  let shift = []
  for (let i = 0; i < 512 / settings.algorithm.pixel.size; i++) {
    shift.push({
      value: 0,
    })
  }
  ...
  _p5.draw = () => {
      ...
      for (let y = 0; y < images[5].height; y += settings.algorithm.pixel.size) {
        ...
        for (let i = 0; i < 10; i++) {
          if (together > i * 26 && together < 26 + i * 26) {
            _p5.image(orderedImages[i + 5], x, y + Math.round(shift[x / settings.algorithm.pixel.size].value / settings.algorithm.pixel.size) * settings.algorithm.pixel.size, settings.algorithm.pixel.size, settings.algorithm.pixel.size)
          }
        }
      }
    }
    ...
    for (let x = 0; x < images[0].width; x += settings.algorithm.pixel.size) {
      for (let y = 0; y < images[0].height; y += settings.algorithm.pixel.size) {
        // _p5.noiseDetail(2, 0.2)
        // noiseVal = _p5.noise(settings.algorithm.target.x + x * settings.algorithm.noise.scale, settings.algorithm.target.y + y * settings.algorithm.noise.scale - _p5.frameCount * settings.algorithm.speed, _p5.frameCount * settings.algorithm.speed)

        const pixelIndex = (x + y * images[0].width) * 4
        const r = images[0].pixels[pixelIndex + 0]
        const g = images[0].pixels[pixelIndex + 1]
        const b = images[0].pixels[pixelIndex + 2]
        ...
        for (let i = 0; i < 10; i++) {
          if (together > i * 26 && together < 26 + i * 26) {
            _p5.image(orderedImages[i + 5], x, y - (512 - Math.round(shift[x / settings.algorithm.pixel.size].value / settings.algorithm.pixel.size) * settings.algorithm.pixel.size), settings.algorithm.pixel.size, settings.algorithm.pixel.size)
          }
        }
      }
    }
